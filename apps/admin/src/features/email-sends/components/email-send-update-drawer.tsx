...
export function EmailSendUpdateDrawer({ send, isOpen, onClose }: { send: EmailSendDto; isOpen: boolean; onClose: () => void }) {
		...
		<Drawer open={isOpen} onOpenChange={(o) => !o && onClose()}>
			<DrawerContent>
				<DrawerHeader>
					...
					<DrawerDescription>{send.email} — {send.type}/{send.status}</DrawerDescription>
				</DrawerHeader>
				<div className="px-4">
