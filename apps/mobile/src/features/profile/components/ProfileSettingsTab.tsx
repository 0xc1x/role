...
import { Bell, ChevronRight, CircleHelp, Heart, MapPin, Settings, Star, User, type LucideIcon } from "lucide-react-native";
...
const SETTINGS_GROUPS: SettingsGroup[] = [
	{
		...
		items: [
			...
			{ icon: MapPin, label: strings.profile.savedAddresses, href: "/profile/addresses" },
		],
	},
	{
		title: strings.profile.sectionPreferences,
		items: [
			{ icon: Bell, label: strings.profile.notifications, href: "/profile/notifications" },
			{ icon: Heart, label: strings.favorites.title, href: "/profile/favorites" },
			{ icon: Star, label: strings.orders.myReviews, href: "/profile/reviews" },
			{ icon: Settings, label: strings.profile.settings, href: "/profile/settings" },
		],
	},
	{
		title: strings.profile.sectionHelp,
		items: [{ icon: CircleHelp, label: strings.profile.help, href: "/profile/help" }],
	},
];

function IconRow({ icon: Icon, label, href, isLast }: SettingsItem & { isLast: boolean }) {
...
export function SettingsTab() {
	const { colors } = useTheme();
	return (
		<View style={[ styles.card, { gap: spacing.lg, backgroundColor: colors.background }]}>
