...
const { OrderCardSkeleton: ConsumerOrderCardSkeleton } = await import(
...
const item: OrderDetail = {
	order: {
		...
		order_number: "20260916-12345678901234567890",
		status: "pending",
		price: 12345,
		...
		net_amount: 11110.5,
		payout_id: null,
		created_at: "2026-09-16T12:30:00",
		updated_at: "2026-09-16T12:30:00",
	},
	offerTitle: "Artisan bread selection with seasonal pastries",
	offerImageUrl: null,
	businessName: "Bakery",
		...
		for (const status of Object.keys(actionCounts) as OrderStatusType[]) {
			const html = renderToStaticMarkup(
				createElement(OrderCard, {
					...
					item: { ...item, order: { ...item.order, status } },
				}),
			);
			const buttons = [
			...
			if (status === "pending" || status === "confirmed")
				expect(buttons[1]).toContain(strings.business.ordersMarkReady);
			if (status === "ready_for_pickup")
				expect(buttons[1]).toContain(strings.business.ordersValidateAndDeliver);
		}
		expect(
			renderToStaticMarkup(createElement(OrderCardSkeleton)),
		).not.toBeEmpty();
	}
});
...
test("order skeletons expose loading without actions and omit active footers for history", () => {
	for (const palette of [light, dark]) {
		...
		for (const [Component, footer] of [
			...
			expect(Component({ active: true }).props.accessibilityState).toEqual({
				busy: true,
			});
			const active = renderToStaticMarkup(
				createElement(Component, { active: true }),
			);
			const history = renderToStaticMarkup(
				createElement(Component, { active: false }),
			);
			...
			const html = renderToStaticMarkup(
				createElement(OrderCard, {
					...
					item: {
						...item,
						customerName: null,
						offerImageUrl: "https://example.invalid/fixture.png",
