...
const presses: any[] = [];
const push = mock(() => {});
const wrapper = ({ children }: Props) => createElement(nativeWeb.View, null, children);
let InteropView: any;
...
const { cssInterop } = await import("react-native-css-interop/dist/runtime/native/api");
InteropView = cssInterop((props: Props) => {
	views.push(props);
	return createElement(nativeWeb.View, props);
}, { className: "style" });
InteropPressable = cssInterop((props: any) => {
	presses.push(props);
	return createElement(nativeWeb.Pressable, { ...props, testOnly_pressed: pressed });
}, { className: "style" });
...
mock.module("@/src/features/auth/store", () => ({ useAuthStore: () => ({ id: "owner" }) }));
mock.module("@/src/features/business/hooks", () => ({
	useBusinesses: () => ({ data: [{ id: "business-1" }] }),
	useBusinessStats: () => ({ data: {
		revenue: 0, revenueChange: 0, ordersCount: 0, ordersChange: 0,
		rescuedCount: 0, rescuedChange: 0, avgRating: 4.5, dailyStats: [], topProducts: [],
	} }),
}));
mock.module("@/src/core/ui", () => ({ AppText: nativeWeb.Text, Screen: wrapper, ScreenHeader: () => null, EmptyState: () => null, ErrorState: () => null }));
mock.module("@/components/ui/skeleton", () => ({ Skeleton: () => null }));
mock.module("@/components/ui/button", () => ({ Button: () => null }));
const { default: BusinessStatsScreen } = await import("../../../../app/business/[id]/stats");

const styleOf = (props: Props) => nativeWeb.StyleSheet.flatten(props.style) ?? {};

test("installed native interop drops Pressable callback styles", () => {
	const callback = mock(() => ({ flexBasis: "47%", flexGrow: 1, flexShrink: 1 }));
...
test("native interop retains two equal KPI columns and full-width review surface", () => {
	...
	for (const column of columns) expect(styleOf(column)).toEqual({ flex: 1, minWidth: 0 });
	// Inspect real Card output after native interop; no Card mock hides its surface or sizing.
	const cards = views.filter((view) => styleOf(view).backgroundColor === light.card);
	expect(cards).toHaveLength(6);
	for (const card of cards.slice(0, 4)) {
		expect(styleOf(card)).toMatchObject({ width: "100%", flexGrow: 1, borderColor: light.borderSolid });
	}
