...
export const Errors = {
	...
	conflict: (message = "Ese elemento ya existe.") =>
		AppError.of("conflict", message),
	businessRule: (message: string, code?: string) =>
		AppError.of("business_rule", message, code),
	unknown: (message = "Algo salió mal. Inténtalo de nuevo.") =>
		AppError.of("unknown", message),
};
