-- Cupones globales de plataforma: business_id NULL = aplicable a ofertas de
-- cualquier negocio (creados desde admin). Los cupones de negocio siguen igual.

ALTER TABLE public.coupons ALTER COLUMN business_id DROP NOT NULL;

-- reserve_offer: el match de cupón acepta cupones del negocio O globales.
-- La precedencia negocio-primero la resuelve el cliente al resolver el código;
-- el RPC valida que el cupón recibido aplique a la oferta del negocio.
